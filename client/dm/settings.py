...
class _RemotesCache(object):
    ...
    def _get_empty_config(self):
        ...
        return {'remotes': {
            'origin': {
                'remote_authentication_url': 'http://127.0.0.1:8000/gettoken/'  # TODO!
            }
        }}
    ...
    def add_or_update_new_remote(self, remote_name, username, team_slugs, token, urls=None):
        urls = urls or dict()
        f = self.get_file()
        if remote_name not in f:
            f[remote_name] = {'urls': dict(), 'teams': dict()}
        f[remote_name]['username'] = username
        f[remote_name]['token'] = token
        for team_slug in team_slugs:
            f[remote_name]['teams'][team_slug]['urls'].update(urls)
        self.save(f)
...
class _Settings(object):
    ...
    def save_token(self, username, team_slugs, token_value):
        ...
        token_handler.add_or_update_new_remote(self.active_remote, username, team_slugs, token_value)
